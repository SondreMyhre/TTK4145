...
- Receives `HallAssignment` from `OrderSync` describing which hall orders this elevator should serve.
...
- Cab orders: `cabOrders [N_FLOORS]bool`
- Assigned hall orders: `assignedHall ...` (matrix or set)
- Local request matrix (if used)
- Door timer resource (timer lives in the **shell**)

No other module is allowed to mutate these structures directly.
...
- `Tick <-chan time.Time` *(optional)*  
  If you prefer driving timers via Tick events.
...
- `Cleared chan<- ClearedOrders`  
  Notifies OrderSync about which orders were cleared.
...
- Pure-ish transition function(s):
  - `Step(state, requests, event) -> (newState, newRequests, effects)`
